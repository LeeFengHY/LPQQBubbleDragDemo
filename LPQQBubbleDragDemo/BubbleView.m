...
- (void)initializerUI
{
    ...
    self.bubbleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.frontView.frame.size.width, self.frontView.frame.size.width)];
    self.bubbleLabel.textColor = [UIColor whiteColor];
    self.bubbleLabel.textAlignment = NSTextAlignmentCenter;
    [self.frontView insertSubview:self.bubbleLabel atIndex:0]; //[self.frontView addSubview:self.bubbleLabel]
...
- (void)displayLinkAction:(CADisplayLink *)disLink
{
    ...
    centerDistance = sqrtf((x2-x1)*(x2 - x1) + (y2 - y1)*(y2- y1)); //d的距离
...
#if 0
        //不用fabsf来计算
        cosDigree = fabsf(y2 - y1)/centerDistance;
        sinDigree = fabsf(x2 - x1)/centerDistance;
#endif
        cosDigree = (y2-y1)/centerDistance;
        sinDigree = (x2-x1)/centerDistance;
    }
    
    r1 = oldBackViewFrame.size.width / 2 - centerDistance / self.viscosity; //慢慢变小
    NSLog(@"%0.2f+++%0.2f",r1,centerDistance);
...
- (void)drawRect{
    ...
    if (backView.hidden == NO) {
        ...
        shapeLayer.fillColor = [fillColorForCute CGColor];//[fillColorForCute CGColor]; //填充色fillColorForCute
